...
export function FinalDraftPage({ auth, onLogout }: Props) {
  ...
  useEffect(() => {
    if (!review && !draft.value && analysisQuery.data?.initial_answer) draft.setValue(analysisQuery.data.initial_answer)
  }, [analysisQuery.data?.initial_answer, review])
    ...
    <main id="main-content" className="page-content revision-layout">
      <section className="revision-main">
        ...
        <form className="answer-form" onSubmit={(event) => { event.preventDefault(); if (!review && !answerError && session) submit.mutate() }}>
          <label htmlFor="final-answer">我的修改稿</label>
          <textarea id="final-answer" value={answer} onChange={(event) => draft.setValue(event.target.value)} readOnly={review} maxLength={12000} rows={14} aria-describedby="final-help final-count final-error" />
          <div className="field-footer"><span id="final-help">{review ? <><CheckCircle2 size={15} />已提交版本，仅供查看</> : <><Save size={15} />草稿已保存在本机</>}</span><span id="final-count">{answer.length} / 12000 字</span></div>
